...
stone = {'камень':2 }
dirt  = {'земля':3 }
"""словарь с предметами и их весом"""

inventory=[]
weight=0
max_weight=10
"""список предметов, вес, максимальный вес"""
...
while (True): 
    ...
    if choise == 1: 
        """проверкка максимального веса и добавление предметов в инвентарь"""
        choise_item= int(input ('1-палка\n2-камень\n3-земля\n'))
        
        if (choise_item == 1) and ((weight + stick.get('палка')
                                             <= max_weight)):
            inventory.append(stick)
            weight=weight + stick.get('палка')

        elif (choise_item==2) and ((weight + stone.get('камень') 
                                                <= max_weight)):
            inventory.append(stone)
            weight=weight + stone.get('камень')

        elif (choise_item==3) and ((weight + dirt.get('земля')
                                                <= max_weight)):    
